...
| Missing-required-$a field removal (also treats a punctuation-only $a, e.g. "." or "--", as missing) | Removed field | Yes (`--no-strip-missing-required-a`) | `removed_missing_a` | FIXED/REQUIRES ATTENTION | **Yes** | `--no-strip-missing-required-a`; `--required-a-tags-file` |
