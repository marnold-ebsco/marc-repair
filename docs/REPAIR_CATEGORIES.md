...
| 852 (Location) missing $h (call number) entirely | Flagged only, field left completely untouched -- nothing to remove | detect-only | `missing_call_number` | INFORMATIONAL | No -- a call number can legitimately be absent (e.g. some serials/electronic holdings), and it can be a large fraction of a file | `--log-missing-call-number` |
